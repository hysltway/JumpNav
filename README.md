...
- 兼容 ChatGPT 新版 `section[data-turn]` 对话结构，导航目录可继续按真实提问与回复顺序生成。
- 优化 ChatGPT 虚拟化滚动场景下的跳转逻辑，优先定位已挂载的提问内容并等待消息重新挂载。
- 为 ChatGPT 消息索引加入会话级文本缓存，减少消息临时卸载导致的目录条目丢失。
- 将导航数量与兜底条目文案从“提示词”调整为“提问”，并同步多语言文案。
