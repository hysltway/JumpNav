...
- 修正 Prompt Library 提示词内容的空白行裁剪逻辑，保留正文内部换行，只移除首尾空行。
- 重构提示词列表项的标题区与正文区交互结构，减少操作按钮与点击注入区域互相挤压的问题。
- 优化提示词编辑输入框滚动条与列表标题换行样式，提升长内容与长标题场景下的可读性和操作稳定性。
- 清理仓库内收尾 skill 副本，统一改为使用全局 `jumpnav-release-sync`，并同步更新版本号与发布文档。
